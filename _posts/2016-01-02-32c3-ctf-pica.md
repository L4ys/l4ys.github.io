...
透過 `ps` 指令可以得知在 `PTY1` 要求輸入 Flag 所執行的程式為 `/usr/bin/verify-flag` :
...
dev_write(struct file *filp, const char *buf, size_t size, loff_t *f_pos) {
    ...
    if ( !copy_from_user(user_input, buf + pos, 45) != 0) {
...
參考 [ACPI 官方 Spec](http://www.acpi.info/spec50a.htm) 逆向成類 C / Python 形式:

```c
unsigned char HEXC[] = {0x37, 0x65, 0x35, 0x66, 0x34, 0x39, 0x63, 0x33, 0x62, 0x32, 0x36, 0x30, 0x61, 0x31, 0x38, 0x64, 0x65, 0x37, 0x39, 0x63, 0x33, 0x62};
unsigned char FLG[] = {0xA4, 0xB3, 0x28, 0x2D, 0x9B, 0x93, 0xFE, 0x35, 0x75, 0xDC, 0x78, 0xB4, 0x3C, 0x81, 0x37};
...
char* HEX(char* hash)
{
    char buf[20] = malloc(20);

    for ( int i = 0; i < 20; ++i ) {
        m = HEXC.index(hash[i], 2) - 2;
        n = HEXC.index(hash[i+20]);
        if ( !m || !n )
            return NULL;
        buf[i] = ( ( m ^ n ) | ( n << 4 ) ) ^ 0xC3
    }
...
總結一下 Flag 要符合的條件: 
 ...
 - 由 `svchost.ko` 得知 `FLAG` method 的回傳值必須為 0 
 - 長度為 45 ， 以 `32C3_` 開頭

`FLAG` method 會將 Flag 後 40 位傳給 `HEX` 做某種運算，得到一組長度 20 位的字串

`HEX` method 中的結果字串 `buf` 需要符合 `buf[0] + buf[1] + buf[2] + buf[3]`，
可以視為 `buf` 的前四位都是 `0x00`

而 `FLAG` method 中的迴圈所計算的總和與 `buf[19]` 所和必須為 0

以這兩個條件可以進一步推出 buf 字串的完整內容為:
`[0,0,0,0,0x70,0xa4,0xb3,0x28,0x2d,0xeb,0x37,0x4d,0x1d,0x58,0x37,0x4f,0xf9,0x21,0xd9,0x0]`

再由 `buf` 的值配合 `z3` 反推出 `HEX` method 中每一輪的 `m` 及 `n`:
透過 m 及 n 配合 `HEXC` array 就可以組合出正確的 Flag:

```py
#!/usr/bin/env python
HEXC = [0x37, 0x65, 0x35, 0x66, 0x34, 0x39, 0x63, 0x33, 0x62, 0x32, 0x36, 0x30, 0x61, 0x31, 0x38, 0x64, 0x65, 0x37, 0x39, 0x63, 0x33, 0x62]
m = [15, 15, 15, 15, 8, 1, 7, 5, 0, 10, 11, 6, 3, 2, 11, 4, 9, 12, 11, 15]
n = [12, 12, 12, 12, 11, 6, 7, 14, 14, 2, 15, 8, 13, 9, 15, 8, 3, 14, 1, 12]
