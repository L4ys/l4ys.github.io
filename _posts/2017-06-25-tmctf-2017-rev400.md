...
BOOL WINAPI MySetCursorPos(int x, int y) // RCX RDX R8 R9
{
    ...
    double min = 9999;
    for ( int j = 0; j < 16; ++j ) {
        double d = dis(x, y, button_pos[j].x + 12.5, button_pos[j].y + 12.5);
        if ( d < min ) {
            min = d;
            min_idx = j;
        }
...
void Hook(DWORD64 dwSource, LPVOID lpTarget, UINT uNops = 0)
{
    ...
    VirtualProtect((LPVOID)dwSource, 10 + uNops, PAGE_READWRITE, &OldProtect);
    ...
    *(LPWORD)(dwSource + 10) = 0xe0ff; // jmp rax
    ...
    VirtualProtect((LPVOID)dwSource, 10 + uNops, OldProtect, &OldProtect2);
}
